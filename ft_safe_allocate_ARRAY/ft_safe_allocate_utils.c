...
/*   Updated: 2025/04/20 18:10:16 by mait-you         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	size_t size[2])
{
	...
	while (i < MAX_ALLOCATIONS)
	{
		...
		{
			ptr_array[i].original_ptr = original_ptr;
			ptr_array[i].user_ptr = user_ptr;
			ptr_array[i].size = size[0] * size[1];
			return (SUCCESS);
		}
		i++;
	}
